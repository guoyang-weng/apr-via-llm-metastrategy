...
def get_votes(task, x, ys, n_evaluate_sample):
    ...
    return vote_prompt, vote_outputs, values
...
def solve(args, task, idx, to_print=True):
    ...
    for step in range(task.steps):
        ...
        if args.method_evaluate == 'vote':
            vote_prompt, vote_outputs, values = get_votes(task, x, new_ys, args.n_evaluate_sample)
        ...
        infos.append({'step': step, 'x': x, 'ys': ys, 'new_ys': new_ys, 'vote_prompt':vote_prompt, 'vote_outputs':vote_outputs, 'values': values, 'select_new_ys': select_new_ys})
        ys = select_new_ys
